...
def predict_output(X_data,preds):
    pred=max_f1_output(X_data,preds)
    str_pred=[]
    products=pd.read_csv('sample_data/products.csv')[['product_id','product_name']]
    products=products.set_index('product_id').T.to_dict('list')
    for idx,row in pred.iterrows():
        ...
        for i in row['products']:
            if i=='None':
                ...
                s.append(products[i][0])
        str_pred.append(",".join(s))
    pred['products will reorder']=str_pred
    pred=pred.drop(['products'],axis=1)
    return pred
...
@app.route('/sample_order_ids')
def sample_order_ids():
    return flask.render_template('sample_order_ids.html')
